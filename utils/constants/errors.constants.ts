...
const AUTH_ERRORS = {
  NEXTAUTH_ERROR_URL: (type: "auth" | "inventory") =>
    `/auth/login?${[SEARCH_PARAMS.ERROR]}=${SEARCH_PARAMS.ERROR}&${[
...
const redirectDynamicUrls = (
  ...
  return {
    ERROR_URL: `${url}?${[SEARCH_PARAMS.ERROR]}=${SEARCH_PARAMS.ERROR}&${[
      SEARCH_PARAMS.ENTITY_TYPE,
    ]}=${type}&${[SEARCH_PARAMS.ERR_MSG]}=${err_msg}.&${[
      SEARCH_PARAMS.ERR_DESC,
    ]}=${err_desc}.&${[SEARCH_PARAMS.BTN_LABEL]}=${err_btn ?? "Try Again"}`,
  };
};
